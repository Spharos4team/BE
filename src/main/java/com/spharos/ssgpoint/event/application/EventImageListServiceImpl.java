...
public class EventImageListServiceImpl implements EventImageListService {
    ...
    public void addEventImageList(EventImageListDao eventImageListDao) {
        Event event = eventRepository.findById(eventImageListDao.getEvent().getId()).orElseThrow(() ->
                new IllegalArgumentException("다음 이미지를 찾을 수 없습니다: " + eventImageListDao.getEvent().getId()));

        EventImage eventImage = eventImageRepository.findById(eventImageListDao.getEventImage().getId()).orElseThrow(() ->
                new IllegalArgumentException("다음 이벤트 이미지를 찾을 수 없습니다: " + eventImageListDao.getEventImage().getId()));
    ...
    public EventImageListDto getEventImageListById(Long id) {
        ...
        EventImage eventImage = eventImageRepository.findById(id).orElseThrow(() ->
                new IllegalArgumentException("EventImage not found with id: " + id));
    ...
    public List<EventImageListDto> getEventImageListByEventId(Long eventId) {
        EventImageList eventImageList = eventImageListRepository.findById(eventId).orElseThrow(() ->
                new IllegalArgumentException("EventImageList not found with id: " + eventId));
        return null;
    }

    @Override
    public List<EventImageListDto> getEventImageListByUserId(Long userId) {
        EventImageList eventImageList = eventImageListRepository.findById(userId).orElseThrow(() ->
                new IllegalArgumentException("EventImageList not found with id: " + userId));
        return null;
    }
}
